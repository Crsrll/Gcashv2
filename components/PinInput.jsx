    ...
    <div className="flex gap-1.5 w-full justify-between">
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          disabled={disabled}
          className={`pin-dot flex-1 min-w-0 max-w-[46px] aspect-square rounded-xl border border-[#E5E7EB] text-center text-xl font-mono bg-white transition-all ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        />
      ))}
    </div>
  )
